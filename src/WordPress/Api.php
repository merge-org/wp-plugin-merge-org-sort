...
final class Api implements ApiInterface {
	...
	public function getSortProduct(int $productId): \MergeOrg\WpPluginSort\Data\Sort\Product {
		$sales = get_post_meta($productId, $this->constants->getSalesMetaKey(), TRUE) ?: [];
	...
	public function getUnrecordedOrders(int $orders = 5): array {
		...
		foreach($orders_ as $order) {
			...
			foreach($order->get_items() as $item) {
				...
				$productSales = get_post_meta($productId, $this->constants->getSalesMetaKey(), TRUE) ?: [];
				$variationId = $data['variation_id'];
				$variationSales =
					$variationId ? (get_post_meta($variationId, $this->constants->getSalesMetaKey(), TRUE) ?: []) : [];
		...
		if($logger) {
			...
			$logger->info("Orders found for cron($time): $ordersForLogging", ['source' => 'merge-org-sort']);
		}
	...
	public function getProductSales(int $productId): array {
		return get_post_meta($productId, $this->constants->getSalesMetaKey(), TRUE) ?: [];
	}
